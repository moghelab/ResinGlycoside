...
class step3:
    ...
    def stats(self,syslist):
        ...
        line1=file1.readline()
        startProcessing=0; m=0; rcount=0; scount=0; oroot=0; oshoot=0; both=0; twofrags=0; threefrags=0; mulfrags=0
        while line1:
            if line1.startswith('Alignment'):
                ...
                #Specify values
                id1=tab1[0]; avgrt=tab1[1]; avgmz=tab1[2]; postCuration=tab1[5]; flag=0
                ...
                #    print (v, tab1[v])
                pa_samples=[float(x) for x in [tab1[22],tab1[23]]] #for GNPS input                
                ...
                if root>1000 and shoot==0:
                    oroot+=1
                elif root==0 and shoot>1000:
                    oshoot+=1
                else:
                    if root>1000 and shoot>1000:
                        both+=1
                ...
                #Get other signature fragments that are atleast 0.20X maxfrag
                for frag in idict:
                    if frag!=maxfrag:
                        intensity=idict[frag]
                        if (intensity/maxint)<=0.20:
                            select.append(frag)
        ...
        out1=open(syslist[1]+".stats",'w')
        out1.write('#python {}\n'.format(' '.join(sys.argv)))
        flist2=[str(x) for x in flist]
        out1.write('#FileName\tSpecies\tTotal\tShoot\tRoot\tOnlyShoot\tOnlyRoot\tBoth\t{}\tTwoFrags\tThreeFrags\tMulFrags\n'.format('\t'.join(flist2)))
        ...
        out1.write(f'{syslist[1]}\t{syslist[2]}\t{total}\t{scount}\t{rcount}\t{oshoot}\t{oroot}\t{both}\t{fline}\t{twofrags}\t{threefrags}\t{mulfrags}\n')
        out1.close()
...
if __name__ == '__main__':
    ...
    step3.stats([fragfile,areafile,species])
    print ("Done!")
