...
while line1:
    if line1.startswith('#'):
        ...
        frags=eval(tab1[3]) #Annotated Frags
        neut=eval(tab1[4]) #Frags used for neutral loss calculations
        unmatched=eval(tab1[5]) #Unmatched frags        
        ...
        explen=len(mlist); texp+=explen
        unexplen=len(unexp)
        total=unexplen+explen; ttot+=total
...
print ("Average, median expl: ", sum(vlist1)/len(vlist1), np.median(vlist1))
print ("Average, median unexpl: ", sum(vlist2)/len(vlist2), np.median(vlist2))
print ("Total expl: ", texp)
print ("Total: ", ttot)
print ("Done!")
